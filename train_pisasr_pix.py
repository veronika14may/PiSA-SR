...
def get_args():
    ...
    p.add_argument("--init_ckpt", default=None,
                   help="опционально: релизный pisa-sr .pkl для дообучения поверх него")
    ...
    p.add_argument("--pos_prompt_csd", default="", help="добавляется к промпту")
    p.add_argument("--neg_prompt", default="")
    # RAM (извлечение тегов-промптов из HQ); без него — фиксированный --prompt
    p.add_argument("--ram_path", default=None, help="ram_swin_large_14m.pth")
    # обучение
    p.add_argument("--output_dir", default="experiments/ft")
...
def main():
    ...
    if args.init_ckpt is not None:
        sd = torch.load(args.init_ckpt, map_location="cpu")["state_dict_unet"]
        msd = net.unet.state_dict()
        loaded = 0
        for k, v in sd.items():
            if k in msd and msd[k].shape == v.shape:
                msd[k].copy_(v); loaded += 1
        print(f"[init_ckpt] загружено LoRA-тензоров: {loaded}")

    net.unet.set_adapter(PIX_ADAPTERS)
    net.set_train_pix()
    net.unet.enable_gradient_checkpointing()

    params = [p for n, p in net.unet.named_parameters() if "lora" in n and p.requires_grad]
    print(f"обучаемых LoRA-параметров: {sum(p.numel() for p in params)/1e6:.2f}M")
    ...
    while step < args.max_steps:
        for batch in dl:
            ...
            with torch.autocast("cuda", dtype=amp_dtype, enabled=use_amp):
                pred, _, _, _ = net(lq, hq, batch=batch, args=args)
            ...
            if step % args.log_steps == 0:
                print(f"step {step}/{args.max_steps}  "
                      f"l2 {loss_l2.item():.4f}  lpips {loss_lp.item():.4f}")
            if step % args.save_steps == 0:
                outf = os.path.join(args.output_dir, "checkpoints", f"pisasr_pix_{step}.pkl")
                net.save_model(outf)
                print("saved", outf)
            if step >= args.max_steps:
                break

    outf = os.path.join(args.output_dir, "checkpoints", "pisasr_pix_final.pkl")
    net.save_model(outf)
    print("done ->", outf)
