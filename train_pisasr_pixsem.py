...
ALL_ADAPTERS = PIX_ADAPTERS + ["default_encoder_sem", "default_decoder_sem", "default_others_sem"]
...
def get_args():
    ...
    p.add_argument("--init_ckpt", default=None, help="опционально: релизный pisa-sr .pkl")
    ...
    # RAM
    p.add_argument("--ram_path", default=None)
    # обучение
    p.add_argument("--output_dir", default="experiments/full")
    p.add_argument("--pix_steps", type=int, default=4000, help="шагов на стадию 1 (pix)")
...
def main():
    ...
    params = [p for n, p in net.unet.named_parameters() if "lora" in n]
    opt = torch.optim.AdamW(params, lr=args.lr)

    csd = CSDLoss(args=args, accelerator=_Acc()); csd.requires_grad_(False)
    ...
    while step < args.max_steps:
        for batch in dl:
            # переключение на стадию 2 (semantic)
            if step == args.pix_steps:
                print(f"=== шаг {step}: переход на семантическую стадию ===")
                net.unet.set_adapter(ALL_ADAPTERS)
                net.set_train_sem()
                lam_l2, lam_lp, lam_csd = args.lambda_l2, args.lambda_lpips, args.lambda_csd
            ...
            with torch.autocast("cuda", dtype=amp_dtype, enabled=use_amp):
                ...
                loss_l2 = F.mse_loss(pred.float(), hq.float()) * lam_l2
                loss_lp = (lpips_fn(pred.float(), hq.float()).mean() * lam_lp
